...
export default function PetRehomeVerifyScreen() {
  ...
  const resolvedSpeciesParam = resolveParam(recognizedSpecies);
  const resolvedBreedCn = resolveParam(recognizedBreedCn);
  const resolvedAge = resolveParam(recognizedAge);
  const resolvedDesc = resolveParam(recognizedDesc);
  const resolvedConfidence = resolveParam(recognizedConfidence);
  const isManual = modeValue === 'manual' && !resolvedPhotoUri;
  const hasRecognition = !isManual && (!!resolvedBreedCn || !!resolvedSpeciesParam);
  ...
  const [recognizedSpeciesState] = React.useState<string | null>(resolvedSpeciesParam);
  const [name, setName] = React.useState('');
  const [age, setAge] = React.useState(hasRecognition && resolvedAge ? resolvedAge : '');
  const [breed, setBreed] = React.useState<string | null>(() =>
    hasRecognition && resolvedBreedCn ? resolvedBreedCn : null,
  );
  const [location] = React.useState<string>(() => locationOptions[0]);
  const [personalityText, setPersonalityText] = React.useState(
    hasRecognition && resolvedDesc ? resolvedDesc : '',
  );
  ...
  // Note: we no longer reset breed if it's not in the hardcoded list,
  // because AI recognition may return breeds outside the predefined options.
  ...
  const handlePublish = async () => {
    ...
    const resolvedSpecies = resolveSpeciesFromBreed(breed, recognizedSpeciesState);
    if (!resolvedSpecies) {
      setSaveError('请选择宠物品种。');
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        {hasRecognition ? (
          <View style={styles.recognitionBanner}>
            <FontAwesome5 name="magic" size={Theme.sizes.s14} color={Theme.colors.textWarmStrong} />
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.recognitionDetail}>
                品种: {resolvedBreedCn ?? '未知'} · 估龄: {resolvedAge ?? '?'}岁
              </Text>
              {resolvedDesc ? (
                <Text style={styles.recognitionDesc}>{resolvedDesc}</Text>
              ) : null}
            </View>
          </View>
        ) : null}
...
function resolveSpeciesFromBreed(breed: string | null, recognizedSpecies?: string | null) {
  if (breed) {
    const catBreeds: readonly string[] = breedOptions.CAT;
    const dogBreeds: readonly string[] = breedOptions.DOG;
  ...
  // If breed is not in the hardcoded lists, fall back to AI-recognized species
  if (recognizedSpecies) {
    const upper = recognizedSpecies.toUpperCase();
    if (upper === 'CAT' || upper === 'DOG') {
      return upper;
    }
  }
  return null;
